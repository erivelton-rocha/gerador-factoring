 ...
 */
public class RGService2 extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String startIndicator = "Numero Nome do devedor Vencimento Valor no Vencimento";
...
//            System.out.println(stringSemEspaco);
    ...
    private List<String[]> reprocessData(List<String> lines) {
        ...
        // Regex com grupos de não-captura para nome e data
        // ^ e $ garantem que a correspondência é da linha inteira
        // Regex com R$ opcional e grupos de não-captura para nome e data
        String regex = "^(\\d{6}-\\d{2})\\s+(?:[\\w\\s./-]+)\\s+(?:\\d{2}/\\d{2}/\\d{2})\\s+(?:R\\$\\s+)?([\\d\\.,]+)$";
        ...
        for (String currentLine : lines) {
            ...
            if (matcher.matches()) {
                ...
                //String date = matcher.group(3); // Data está no Grupo 2
                String value = matcher.group(2); // Valor está no Grupo 3

                // Cria um array APENAS com Título, Data e Valor
                String[] record = {title, value};
