 ...
 */
public class SBService2 extends AbstractBankService {
...
//            System.out.println(stringSemEspaco);
            ...
            //boolean header2 = line.toLowerCase().contains("sacado");
            boolean header3 = line.toLowerCase().contains("vencimento");
            boolean header4 = line.toLowerCase().contains("valor");

            if (header1  && header3 && header4) {
                capture = true;
    ...
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        // Regex mais robusto para capturar documento, parcela e valor após a data
        String regex = "(\\d{6}-\\d{2})\\s+.+?\\s+(\\d{1,3}(?:\\.\\d{3})*,\\d{2})";
        ...
        for (String line : lines) {
            ...
            if (matcher.find()) {
                ...
                String parcela = documento.substring(documento.length() - 2); // Extrai a parcela
                   // Captura a data
                String valor = matcher.group(2);    // Captura o valor
