 ...
 */
public class MarcapitalService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIndicator = "O presente Termo Aditivo é formalizado";

        for (String line : text.split("\\n")) {
             line = line.trim().replaceAll("\\t", " ");

            // validar o inicio dos titulos
            Boolean header1 = line.contains("Dcto");
            Boolean header2 = line.contains("Tipo");
            Boolean header3 = line.contains("CNPJ - CPF");
            Boolean header4 = line.contains("Vlr Face");
    ...
    private List<String[]> reprocessData(List<String> lines) {
        ...
        String regex = "(\\d{6})-\\s+.*?(\\d{2}/\\d{2}/\\d{4})\\s+([\\d.]+,\\d{2})";
        ...
        for (String line : lines) {
            ...
            while (matcher.find()) {
                ...
                String id = matcher.group(1).trim();              // Captura o ID
                String dataVencimento = matcher.group(2);         // Captura a data
                String valor = matcher.group(3);                  // Captura o valor

                String[] record = {id, dataVencimento, valor};
                data.add(record);
            }
        }
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    TituloInfo info = getInfobyTituloVencimento(titulo, dataVencimento);
...
//                            decimalFormat.format(valorBruto), decimalFormat.format(iof), decimalFormat.format(juros), decimalFormat.format(tarifa), decimalFormat.format(valorLiquido));
                    writer.printf("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s%n",
                            empresa, titulo, serie, parcelaBanco, nossoNumero, contrato, codigoBanco,
                            valorBruto,
                            iof,
                            juros,
                            tarifa,
