 ...
 */
public class BelaVistaService extends AbstractBankService {
...
//            System.out.println(stringSemEspaco);
            ...
            Boolean documento = line.contains("Documento");
    ...
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{6})-(\\d{2}).*?(\\d{2}/\\d{2}/\\d{4})?.*?(?<!\\d)(\\d{1,3}(?:\\.\\d{3})*,\\d{2}|\\d{1,3}(?:\\.\\d{3})*\\.?\\d{2}|\\d+\\s\\d{2}|\\d+)$";

        Pattern recordPattern = Pattern.compile(regex);

        for (String line : lines) {
            Matcher matcher = recordPattern.matcher(line.trim().replaceAll("[\\.'\\s]+$", "")); // Remove ponto final

            if (matcher.find()) {
                String documento = matcher.group(1);   // Documento do ID
                String parcela = matcher.group(2);     // Parcela do ID
                String rawValue = matcher.group(4).replace(" ", "");

                // Correct formatting of the value
                String valor;
                if (rawValue.contains(",") && rawValue.indexOf(",") == rawValue.length() - 3) {
                    // Já está no formato correto
                    valor = rawValue.replace(".", "");
                } else if (rawValue.matches("\\d+(\\.\\d{3})*\\.\\d{2}")) {
                    // Se existe um ponto como decimal
                    valor = rawValue.replaceFirst("\\.(?=\\d{2}$)", ",");
                    valor = valor.replace(".", "");
                } else if (rawValue.matches("\\d+")) {
                    // Sem pontos, presume-se sem separador decimal
                    valor = rawValue.substring(0, rawValue.length() - 2) + "," + rawValue.substring(rawValue.length() - 2);
                } else if (rawValue.matches("\\d+\\s\\d{2}")) {
                    // Com espaço, ajusta para vírgula
                    valor = rawValue.replace(" ", ",");
                } else {
                    valor = rawValue; // Fallback para qualquer caso não tratado
                }

                String date = matcher.group(3) != null ? matcher.group(3) : "Data não disponível";
