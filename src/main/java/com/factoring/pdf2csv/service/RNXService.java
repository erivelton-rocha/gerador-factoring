 ...
 */
public class RNXService extends AbstractBankService {
...
//            System.out.println(stringSemEspaco);
    ...
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{5})\\s?(\\d)-?(\\d{2})\\s+.*?(\\d{2}/\\d{2}/\\d{4})\\s+([\\d\\.,]+)";
        ...
        for (String line : lines) {
            ...
            if (matcher.matches()) {
                String documentoParte1 = matcher.group(1); // Primeira parte do Documento do ID
                String documentoParte2 = matcher.group(2); // A parte que pode estar separada
                String parcela = matcher.group(3);         // Parcela do ID
                      // Data
                String valor = matcher.group(5);           // Valor

                // Remove espaços extras
                String documento = documentoParte1 + documentoParte2;
