 ...
 */
public class MultiplicaService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String startIndicator = "Nosso Número CNPJ Sacado Valor Data Vencimento";
        String endIndicator = "3 – As condições da presente cessão de créditos são as seguintes:";
...
//            System.out.println(stringSemEspaco);
    ...
    private List<String[]> reprocessData(List<String> lines) {
        ...
        String regex = "(\\d{5}-\\d)\\s+(\\d{2}\\.\\d{3}\\.\\d{3}/\\d{4}-\\d{2})\\s+R\\$\\s+([\\d.]+,\\d{2})\\s+(\\d{2}/\\d{2}/\\d{4})";
        ...
        for (String currentLine : lines) {
            ...
            if (matcher.matches()) {
                ...
                String date = matcher.group(4); // Data está no Grupo 2
                String value = matcher.group(3); // Valor está no Grupo 3
