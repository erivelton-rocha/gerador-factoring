 ...
 */
public class ViaCapitalService extends AbstractBankService {
...
//            System.out.println(stringSemEspaco);
            ...
            Boolean header1 = line.contains("Número do título");
            Boolean header2 = line.contains("Sacado");
            Boolean header3 = line.contains("Vencimento");
            Boolean header4 = line.contains("Valor");
...
// extrair os dados dos titulos
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{6})-(\\d{2})\\s.*?(\\d{1,3}(?:\\.\\d{3})*,\\d{2})\\s(\\d{2}/\\d{2}/\\d{4})";
        ...
        for (String line : lines) {
            ...
            if (matcher.find()) {
                ...
                String valor = matcher.group(3);      // Valor
                String date = matcher.group(4);       // Data
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    Double valorBruto = Double.parseDouble(row[2].replace(".", "").replace(",","."));
