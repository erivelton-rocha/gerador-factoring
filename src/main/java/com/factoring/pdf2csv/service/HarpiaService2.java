 ...
 */
public class HarpiaService2 extends AbstractBankService {
...
//            System.out.println(stringSemEspaco);
            ...
            Boolean documento = line.contains("Número");
            Boolean sacado = line.contains("Nome do devedor");
            Boolean vencimento = line.contains("Vencimento");
            Boolean valor = line.contains("Valor no Vencimento");
    ...
    public List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{6})-(\\d{2})\\s+.*?(\\d{2}/\\d{2}/\\d{4})\\s+R?\\$\\s?(\\d{1,3}(?:,\\d{3})*\\.\\d{2})";
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            // Verifica se o próximo ID começa na mesma linha ou se o valor está incompleto
            Matcher idMatcher = Pattern.compile("\\d{6}-\\d{2}").matcher(currentLineBuilder.toString());
            boolean isIncomplete = line.matches(".*R\\$\\s?$");

            if (idMatcher.find() && idMatcher.hitEnd() && !isIncomplete) {
                processRecord(currentLineBuilder.toString(), recordPattern, data);
                currentLineBuilder.setLength(0); // Limpa o construtor para o próximo registro
            }
        }

        // Verifica se há uma linha restante a ser processada
    ...
    private void processRecord(String line, Pattern recordPattern, List<String[]> data) {
        ...
        while (matcher.find()) {
            ...
            // Trocar vírgula por ponto e ponto por vírgula
            valor = valor.replace(',', ';')
                    .replace('.', ',')
                    .replace(';', '.');

            String[] record = {documento, parcela, date, valor};
            data.add(record);
        }
    }
