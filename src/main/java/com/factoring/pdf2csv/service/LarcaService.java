 ...
 */
public class LarcaService extends AbstractBankService {
...
//            System.out.println(stringSemEspaco);
    ...
    private List<String[]> reprocessData(List<String> lines) {
        ...
        String regex = "(\\d{6}-\\d{2})\\s.*\\s(\\d{2}/\\d{2}/\\d{4})\\s(\\d{1,3}(\\.\\d{3})*,\\d{2})";

        Pattern recordPattern = Pattern.compile(regex); //^(\d{8})\s+(.+)\s+(\d{2}\/\d{2}\/\d{4})\s+([0-9.,]+)$
        ...
        for (String currentLine : lines) {
            ...
            if (matcher.matches()) {
                ...
                String id = matcher.group(1);    // Captura os 8 dígitos iniciais
                //String texto = matcher.group(2);  // Captura o texto intermediário
                //String date= matcher.group(3);   // Captura a data
                String valor = matcher.group(3);  // Captura o valor
                // Cria um array APENAS com Título, Data e Valor
                String[] record = {id, valor};
