 ...
 */
public class BanestesService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIndicator = "*** MOVIMENTO";

        for (String line : text.split("\\n")) {
             line = line.trim().replaceAll("\\t", " ");
//            System.out.println(stringSemEspaco);
            // validar o inicio dos titulos
            Boolean header1 = line.contains("NOME OU CPF DO SACADO");
            Boolean header2 = line.contains("DADOS DO TITULO");
            Boolean header3 = line.contains("VENCIMENTO");
            Boolean header4 = line.contains("VALOR TITULO");
    ...
    public List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "\\d{2}\\s*DD\\s*\\d{10}\\s*\\w[\\w\\s]+\\d{9}\\s+([A-Z0-9]{9})\\s+(\\d{2}/\\d{2}/\\d{4})\\s+([-]?\\d{1,3}(?:\\.\\d{3})*,\\d{2})\\s+([-]?\\d{1,3}(?:\\.\\d{3})*,\\d{2})\\s+([-]?\\d{1,3}(?:\\.\\d{3})*,\\d{2})\\s+([-]?\\d{1,3}(?:\\.\\d{3})*,\\d{2})";
        ...
        for (String currentLine : lines) {
            ...
            while (matcher.find()) {
                ...
                String valorTitulo = matcher.group(3).replace(".", "").replace(",", ".");
                String jurosMora = matcher.group(4).replace(".", "").replace(",", ".");
                String iof = matcher.group(5).replace(".", "").replace(",", ".");
                String tarifa = matcher.group(6).replace(".", "").replace(",", ".");

                // Converter valor do título para float para verificar se é negativo
                double valor = Double.parseDouble(valorTitulo);

                if (valor >= 0) {
                    // Transformando valores em positivos
                    double jurosMoraPositivo = Math.abs(Double.parseDouble(jurosMora));
                    double iofPositivo = Math.abs(Double.parseDouble(iof));
                    double tarifaPositiva = Math.abs(Double.parseDouble(tarifa));

                    String[] record = {id, date, valorTitulo,
                            String.valueOf(jurosMoraPositivo),
                            String.valueOf(iofPositivo),
                            String.valueOf(tarifaPositiva)};
                    data.add(record);
                }
            }
        }
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    Double valorBruto = Double.parseDouble(row[2]);

                    // iof
                    Double iof = Double.parseDouble(row[4]);

                    //tarifa
                    Double tarifa = 0.0;//Double.parseDouble(row[5]);
                    
                    // Juros
                    Double juros = Double.parseDouble(row[3]);
                    
                    // valor liquido
                    Double valorLiquido = valorBruto - juros - iof - tarifa;
