 ...
 */
public class AtlantaService2 extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        //String startIndicator = "Título Sacado Vencto Vr.de Face Desc.Comcial Vr.Negociado";

        String endIndicator = "Bordero:";
        ...
        for (String line : text.split("\\n")) {
            ...
            boolean temTitulo = line.contains("Título");
            boolean temSacado = line.contains("Sacado");
            boolean temVencto = line.contains("Vencto");
            boolean temVrDeFace = line.contains("Vr.de Face");

//            System.out.println(stringSemEspaco);
            if (temTitulo && temSacado && temVencto && temVrDeFace) {
                capture = true;
    ...
    private List<String[]> reprocessData(List<String> lines) {
        // A lista armazenará arrays com 3 elementos: Título (Identificador), Data, Valor
        List<String[]> data = new ArrayList<>();

        // NOVO Padrão para identificar linhas que contêm Identificador, Data e Valor,
        // opcionalmente com Nome/Descrição no meio.
        // Captura: Grupo 1 (Identificador/Título), Grupo 2 (Data), Grupo 3 (Valor)
        String regex = "(\\d{6}-\\d{2})\\s+.*?\\s(\\d{2}/\\d{2}/\\d{2})\\s+[\\d.,: ]+\\s+[\\d.,: ]+\\s+([\\d.,: ]+)$";
        Pattern recordPattern = Pattern.compile(regex);

        // Iterar por CADA linha
        for (String currentLine : lines) {
            Matcher matcher = recordPattern.matcher(currentLine);

            // Verifica se a linha atual corresponde a QUALQUER UM dos padrões
            if (matcher.matches()) {
                // Extrai as partes do registro usando os grupos de captura
                String title = matcher.group(1); // Título (Identificador) está no Grupo 1
                String date = matcher.group(2); // Data está no Grupo 2
                String value = matcher.group(3).trim(); // Valor está no Grupo 3

                // Cria um array APENAS com Título, Data e Valor
                String[] record = {title, date, value};

                // Adiciona o registro à lista
                data.add(record);

                // Não precisamos pular linhas, pois cada linha de dados é processada individualmente
            }
            // Linhas que não correspondem a nenhum dos padrões (como linhas de Título antigo, Categoria, ou vazias)
            // serão simplesmente ignoradas.
        }
