 ...
 */
public class PremiumService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIndicator = "3. As condições da presente cessão de direitos creditórios";

        for (String line : text.split("\n")) {
            boolean header1 = line.contains("Número");
            boolean header2 = line.contains("devedor");
    ...
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{5,6})-(\\d{1,2})\\s+.*?\\s+([\\d.]+,\\d{2})";
