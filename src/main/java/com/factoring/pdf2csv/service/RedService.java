 ...
 */
public class RedService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIndicator = "3. As condições da presente cessão de Direitos Creditórios são as seguintes:";

        for (String line : text.split("\\n")) {

            boolean header1 = line.contains("Nome do devedor");
            boolean header2 = line.contains("Vencimento");
            boolean header3 = line.contains("Valor no Vencimento");
...
//            System.out.println(stringSemEspaco);
    ...
    public List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{6})N(\\d{2})\\s.*?(\\d{2}/\\d{2}/\\d{4})\\sR\\$\\s(\\d{1,3}(?:,\\d{3})*\\.\\d{2})";
        Pattern recordPattern = Pattern.compile(regex);

        StringBuilder currentLineBuilder = new StringBuilder();

        for (String line : lines) {
            currentLineBuilder.append(line.trim()).append(" "); // Adiciona espaço como separador

            String content = currentLineBuilder.toString();
            Matcher matcher = recordPattern.matcher(content);

            while (matcher.find()) {
                String documento = matcher.group(1);  // Documento
                String parcela = matcher.group(2);    // Parcela
                ...
                // Remove o registro processado do builder
                currentLineBuilder.delete(0, matcher.end());
            }
        }
