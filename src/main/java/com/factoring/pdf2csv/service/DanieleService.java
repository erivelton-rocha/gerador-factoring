 ...
 */
public class DanieleService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIdicator2 = "3. As condigSes da presente cessdo de Direitos Creditérios sdo as seguintes:";

        for (String line : text.split("\n")) {
            // confirmar inicio dos registros
            Boolean snumero = line.contains("Número");
            Boolean snomeDevedor = line.contains("Nome do devedor");
            Boolean svenvimento = line.contains("Vencimento");
            Boolean svalorNoVencimento = line.contains("Valor no Vencimento");


            if (snumero && snomeDevedor && svenvimento && svalorNoVencimento) {
                capture = true;
            ...
            if (line.contains(endIndicator)|| line.contains(endIdicator2)) {
                capture = false;
    ...
    private List<String[]> reprocessData(List<String> lines) {
        ...
        for (String line : lines) {
            ...
            if (line.matches("^\\d{6}[-/]\\d{2}.*")) {
                if (currentRecord.length() > 0) {
                    // Split concatenated records
                    String[] splitRecords = currentRecord.toString().split("(?=\\d{6}-\\d{2})");
    ...
    private String[] parseRecord(String record) {
        ...
        String[] documento = parts[0].split("-|N|/");
        ...
        return new String[]{titulo, parcela, sacado.toString().trim(), vencimento, moeda, valor};
    }
