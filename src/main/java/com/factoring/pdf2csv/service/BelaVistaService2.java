 ...
 */
public class BelaVistaService2 extends AbstractBankService {
...
//            System.out.println(stringSemEspaco);
    ...
    private List<String[]> reprocessData(List<String> lines) {
        ...
        StringBuilder linhaIncompleta = new StringBuilder();

        for (String linha : lines) {
            linha = linha.trim();
            linhaIncompleta.append(" ").append(linha); // Acumula a linha

            matcher = pattern.matcher(linhaIncompleta.toString());

            if (matcher.find()) {
                String documento = matcher.group(1); // Captura o documento
                String parcela = matcher.group(2);   // Captura a parcela

                String valor = matcher.group(5) != null ? matcher.group(5) : ""; // Captura o valor

                // Adiciona o registro ao resultado
                String[] record = {documento, parcela, valor};
                data.add(record);

                // Limpa o StringBuilder após capturar os dados
                linhaIncompleta.setLength(0);
            }
        }

        // Se ainda houver uma linha incompleta no final
        if (linhaIncompleta.length() > 0) {
            System.out.println("Linha incompleta final: " + linhaIncompleta.toString());
        }
