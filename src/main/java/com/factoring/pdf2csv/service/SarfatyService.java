 ...
 */
public class SarfatyService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        for (String line : text.split("\n")) {
            boolean header1 = line.toLowerCase().contains("documento");
            boolean header2 = line.toLowerCase().contains("vencimento");
            boolean header3 = line.toLowerCase().contains("valor(r$)");
    ...
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{6})-(\\d{2})\\s+(\\d{2}\\.\\d{3}\\.\\d{3}/\\d{4}-\\d{2})?\\s*-?\\s*(.*?)\\s+(\\d{2}/\\d{2}/\\d{4})\\s+([\\d.]+,[\\d]{2})\\s+([\\d,]+)";
        ...
        for (String line : lines) {
            ...
            Matcher matcher = Pattern.compile(regex).matcher(currentLine.toString());

            if (matcher.find()) {
                String document = matcher.group(1);  // Documento
                String parcel = matcher.group(2);    // Parcela
                String cnpj = matcher.group(3) != null ? matcher.group(3) : ""; // CNPJ
                String description = matcher.group(4).trim(); // Descrição
                String date = matcher.group(5); // Data
                String value = matcher.group(6); // Valor
                String percentage = matcher.group(7); // Percentagem
                ...
                currentLine.setLength(0); // Limpa o StringBuilder após capturar os dados
            }
        }
...
//                        System.out.println("Linha mal formatada: " + String.join(" | ", row));
                    ...
                    BigDecimal juros = new BigDecimal(row[3].replace(".", "").replace(",", "."));
                    BigDecimal valorBruto = new BigDecimal(row[2].replace(".", "").replace(",", "."));
                    ...
                    TituloInfo info = getTituloInfoByParcela(titulo, Integer.parseInt(parcela), codigoBanco);
