 ...
 */
public class SafraService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String startIndicator = "Seq. Sacado CPF/CNPJ S. Número N. Fiscal Emissão Tipo Aceite Valor Título Vencimento";
        String endIndicator = "Local e data";

        int skipLine = 0;
        for (String line : text.split("\n")) {
            if (line.contains(startIndicator) && (!capture)) {
                capture = true;
                continue; // Skip the start indicator line
            }
            if (line.isBlank() || line.isEmpty() || !capture) {
                continue;
            }
            ...
            if (line.trim().equals("EM")) {
                continue;
            }
            if (line.contains("CENTRAL DE SUPORTE A PESSOA JURÍDICA SAC E DEFICIENTES AUDITIVO / FALA OUVIDORIA")) {
                skipLine = 5;
                continue;
            }
    ...
    private String[] parseRecord(String record) {
        ...
        String documento = parts[parts.length - 6];
        String vencimento = parts[parts.length - 1];
        String moeda = "";
        String valor = parts[parts.length - 2];
        ...
        return new String[]{documento, sacado.toString().trim(), vencimento, valor};
    }
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    BigDecimal jurosLiquido = juros.subtract(valorTarifa).subtract(valorIof);
                    // Calcular o valor líquido
                    BigDecimal valorLiquido = valorBruto.subtract(valorIof).subtract(valorTarifa).subtract(jurosLiquido);
