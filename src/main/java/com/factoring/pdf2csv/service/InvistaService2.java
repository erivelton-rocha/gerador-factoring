 ...
 */
public class InvistaService2 extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String startIndicator = "Documento Sacado Vencimento Valor(R$)";
        String endIndicator = "Qtd de Títulos:";

        for (String line : text.split("\\n")) {

            String trimmedLine = line.trim();
            // Verifica se a linha termina com ',', '.' ou ';' e remove
            if (trimmedLine.endsWith(",") || trimmedLine.endsWith(".") || trimmedLine.endsWith(";")) {
                trimmedLine = trimmedLine.substring(0, trimmedLine.length() - 1);
            }

//            System.out.println(stringSemEspaco);
            if (trimmedLine.contains(startIndicator)) {
                capture = true;
                continue; // Skip the start indicator line
            }
            if(trimmedLine.contains("Página:"))
                continue;
            if (!capture) {
                continue; // skip if not find start indicator
            }
            if (trimmedLine.isBlank() || trimmedLine.isEmpty()
            || trimmedLine.equals("R$")) {
                continue;
            }
            if (trimmedLine.contains(endIndicator)) {
                capture = false;
    ...
    public List<String[]> reprocessData(List<String> lines) {
        ...
        // Regex para capturar ID, Data e Valor
        String regex = "(\\d{6}-\\d{2})\\s+(.*?)\\s+(\\d{2}/\\d{2}/\\d{4})\\s+([\\d.,]+)";
        Pattern pattern = Pattern.compile(regex);

        // Combinar todas as linhas, tratando continuidade quebra de linha
        StringBuilder combinedLines = new StringBuilder();
        for (String line : lines) {
            // Se a linha não contém uma data ou valor, é uma continuação da anterior
            if (!line.matches(".*\\d{2}/\\d{2}/\\d{4}.*")) {
                combinedLines.append(line.trim()).append(" ");
            } else {
                combinedLines.append(line).append(" ");
            }
        }

        // Ajustar texto para regex adequado
        Matcher matcher = pattern.matcher(combinedLines.toString());

        while (matcher.find()) {
            String[] match = new String[3];
            match[0] = matcher.group(1); // ID
            match[1] = matcher.group(3); // Data
            match[2] = matcher.group(4); // Valor
            data.add(match);
        }
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    Double valorBruto =  Double.parseDouble(valorStr.replace(".", "").replace(",", "."));

                    Double iof = valorBruto * (iofRate / 100);
                    Double tarifa = fee;
                    ...
                    // Calcular juros
                    Double juros = valorBruto * taxaDesagio;

                    // Calcular o valor líquido
                    Double valorLiquido =  valorBruto - iof - tarifa - juros;
