 ...
 */
public class BBService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIndicator = "Valor total - R$ Quant";

        for (String line : text.split("\\n")) {
             line = line.trim().replaceAll("\\t", " ");
//            System.out.println(stringSemEspaco);
            boolean header1 = line.contains("Nº do título (**)");
            boolean header2 = line.contains("Valor - R$");
    ...
    // reprocessar registros relevantes
   public List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{20})\\s+(\\d{1,3}(?:\\.\\d{3})*,\\d{2})\\s+(\\d{2}/\\d{2}/\\d{4})";
        ...
        for (String currentLine : lines) {
            ...
            while (matcher.find()) {
                ...
                String id = fullId.substring(3);
                String value = matcher.group(2);
                String date = matcher.group(3);

                String[] record = {id, date, value};
                data.add(record);
            }
        }
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    BigDecimal iof = BigDecimal.valueOf(iofRate/totalRows);
                    BigDecimal tarifa = BigDecimal.valueOf(fee/totalRows);

                    // calcular iof e tarifa por titulo
                    ...
                    TituloInfo info = getTituloByNbco(nbco, codigoBanco);

                    String empresa = info != null ? String.valueOf(info.getCodigoEmpresa()) : "000";
                    String parcela = info != null ? info.getCodigoParcela() : "";
                    String serie = info != null ? info.getSerie() : "";
                    String titulo = info != null? info.getTitulo() : "";
