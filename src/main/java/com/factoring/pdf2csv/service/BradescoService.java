 ...
 */
public class BradescoService extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String startIndicator1 = "Número Número (R$)";
        String startIndicator2 = "Vencimento Parcela Nosso Número Seu Número Sacado Situação Valor (R$)";
        String endIndicator = "Contratos";

        for (String line : text.split("[\n\r]")) {
            System.out.println("linha a processar: " + line);
            if (line.contains(startIndicator1) || line.contains(startIndicator2)) {
                capture = true;
            ...
            if (line.equalsIgnoreCase("A")
                    || line.equalsIgnoreCase("vencer")
                    || line.equalsIgnoreCase("COND VENCER")) {
                continue;
            ...
            if (capture) {
                System.out.println("☑ ️processou alinha a processar: " + line);
        ...
        for (String line : lines) {
            ...
            if (line.matches("\\d{2}/\\d{2}/\\d{4}.*")) {
                if (currentRecord.length() > 0) {
                    ...
                    String[] aux = currentRecord.toString().split("\\+s");
                    for (String record : aux) {
                        data.add(parseRecord(record.trim()));
                    }
                }
                currentRecord.setLength(0); // Clear the current record
            } else if (!hasAtLeastTwoValues(line)) {
                System.out.println("Linha inválida ignorada: " + line);
                continue; // Skip invalid lines
            }

            currentRecord.append(line).append(" ");
            System.out.println("parse: " + currentRecord);
        }

        // Add the last record
        if (currentRecord.length() > 0) {
            // Split concatenated records
            String[] splitRecords = currentRecord.toString().split("\\+s");
    ...
    private String[] parseRecord(String record) {
        ...
        System.out.println("posicao 0: " + parts[0]);
        System.out.println("posicao 1:" + parts[1]);

        System.out.println("posicao 2:" + parts[2]);
        System.out.println("posicao 3:" + parts[3]);

        String documento = parts[3]; // separar parcela e titulo documento, exemplo: 13345N01
        String[] partDocumento = documento.split("N|-|D");

        //  titulo
        String titulo;
        titulo = partDocumento[0];

        // parcela
        String parcela;
        parcela = partDocumento[1];

        String vencimento = parts[0];
        String valorBruto = parts[parts.length - 1];
...
//                        System.out.println("Linha mal formatada: " + String.join(" | ", row));
                    ...
                    // IOF por titulo
                    BigDecimal iofPorTitulo = BigDecimal.ZERO;
                    iofPorTitulo = iof.divide(totalRegistros, 15, RoundingMode.HALF_UP);
                    ...
                    BigDecimal valorLiquido = valorBruto.subtract(juros)
                            .subtract(tarifaPorTitulo)
                            .subtract(iofPorTitulo)
