 ...
 */
public class ATFService extends AbstractBankService {
    ...
    private void reprocessTxt(File txtFile, File reprocessedFile) throws IOException {
        try (PrintWriter writer = new PrintWriter(new FileWriter(reprocessedFile, StandardCharsets.UTF_8))) {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIndicator = "3.   Apuração do valor devido na aquisição dos Direitos de Crédito";

        for (String line : text.split("\n")) {
            boolean header1 = line.contains("Documento");
            boolean header2 = line.contains("Sacado");
            boolean header3 = line.contains("Vencimento");
            if (header1 && header2 && header3 && (!capture)) {
                capture = true;
    ...
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{6})-(\\d{1,2})\\s+.*?\\s+([\\d.]+,\\d{2})";
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    String valorStr = row[2].replace(".","").replace(",", ".");//
                    BigDecimal valorBruto = new BigDecimal(valorStr);

                    BigDecimal iof =  BigDecimal.valueOf(iofRate).divide(BigDecimal.valueOf(totalRows), 10, RoundingMode.HALF_UP);
                    BigDecimal tarifa =  BigDecimal.valueOf(fee).divide(BigDecimal.valueOf(totalRows), 10, RoundingMode.HALF_UP);
