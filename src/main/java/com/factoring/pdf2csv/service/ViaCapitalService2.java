 ...
 */
public class ViaCapitalService2 extends AbstractBankService {
...
//            System.out.println("Dados reprocessados: " + data.size() + " linhas");
    ...
    private List<String> extractRelevantData(String text) {
        ...
        String endIndicator = "Digitado por:";

        for (String line : text.split("\\n")) {
             line = line.trim();
//            System.out.println(stringSemEspaco);
            // validar o inicio dos titulos
            Boolean header1 = line.contains("Id. Sacado");
            Boolean header2 = line.contains("Nome Sacado");
            Boolean header3 = line.contains("Ctl  Cmp Bco");
            Boolean header4 = line.contains("Agen Conta");
            ...
            || line.equals("R$")|| line.contains("Página:")|| line.contains("Pág.:")) {
                continue;
            }
    ...
    private List<String[]> reprocessData(List<String> lines) {
        List<String[]> data = new ArrayList<>();
        String regex = "(\\d{6})-(\\d{2})\\s(\\d{2}/\\d{2}/\\d{4}).*?(\\d{1,3}(?:\\.\\d{3})*,\\d{2})\\s+(\\d{1,3}(?:\\.\\d{3})*,\\d{2})\\s+(\\d{1,3}(?:\\.\\d{3})*,\\d{2})$";
        ...
        for (String line : lines) {
            ...
            if (matcher.find()) {
                ...
                String date = matcher.group(3);       // Data
                String valor = matcher.group(4);      // Valor
                String juros = matcher.group(5);      // Juros
                String valorLiquido = matcher.group(6); // Valor líquido

                String[] record = {documento, parcela, date, valor, juros, valorLiquido};
                data.add(record);
            } else {
...
//                    String parcela = documentoParts.length > 1 ? documentoParts[1].substring(0, 2) : ""; // Corrigir a extração da parcela
                    ...
                    String valorStr = row[3].replace(".","").replace(",",".");
                    String sjuros = row[4].replace(".","").replace(",",".");
                    String svalorLiquido = row[5].replace(".","").replace(",",".");
                    ...
                    BigDecimal valorBruto = new BigDecimal(valorStr); // valor bruto do titulo
                    BigDecimal juros = new BigDecimal(sjuros); // var auxiliar para calcular juro do titulo
                    BigDecimal auxTarifa = BigDecimal.valueOf(fee); //var auxiliar tarifa
                    BigDecimal totalRegistros = BigDecimal.valueOf(totalRows); // total de titulos do arquivo
                    BigDecimal liquidoTitulo = new BigDecimal(svalorLiquido); // liquido informado no arquivo

                    // tarifa
                    BigDecimal tarifa = auxTarifa.divide(totalRegistros,5,RoundingMode.HALF_UP);



                    BigDecimal valorLiquido = valorBruto.subtract(tarifa).subtract(juros);
